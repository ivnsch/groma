...
class ListItemProviderMock: ListItemProvider {
    ...
    init() {
        ...
        let notDone = self.productsVar[0...i].map {ListItem(done: false, product: $0, section: Section(name: "test"))}
        let done = self.productsVar[i...self.productsVar.count - 1].map {ListItem(done: true, product: $0, section: Section(name: "test"))}
