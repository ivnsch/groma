...
//    public var cellRadiusRatio: CGFloat      = 0.7
    ...
    @IBInspectable public var color: UIColor = UIColor.randomColor() {
...
class CircleLiquidBaseView : ActionBarBaseView {
    ...
    let openDuration: CGFloat  = 0.5
    let closeDuration: CGFloat = 0.5
    let viscosity: CGFloat     = 0.3
...
//            let distance = (cell.frame.height * 0.5 + CGFloat(i + 1) * cell.frame.height * 1.5) * (1 - ratio)
            let distance: CGFloat = 140 * CGFloat(i + 1) * (1 - ratio)
