        ...
        doInWriteTransaction ({[weak self] realm in
            if let weakSelf = self {
                ...
                return syncedRet(weakSelf) {
                    ...
                    let productNames = products.map{$0.item.name}
                    ...
                    for product in products {
                        if let existingPlanItem = existingPlanItemsDict[product.item.name] {
...
//                    let mapper = {HistoryItemMapper.historyItemWith($0)} // TODO loading shared users (when there are shared users) when accessing, crash: BAD_ACCESS, re-test after realm update
                    let dbHistoryItems: [HistoryItem] = weakSelf.loadSync(realm, predicate: NSPredicate(format: "addedDate >= %@", Date().startOfMonth as CVarArg), sortDescriptor: nil)
        ...
        doInWriteTransaction ({[weak self] realm in
            if let weakSelf = self {
                ...
                return syncedRet(weakSelf) {
                    ...
                    let productNames = planItems.map{$0.product.item.name}
                    let productNamesStr: String = productNames.map{"'\($0)'"}.joined(separator: ",")
                    ...
                    for planItemInput in planItems {
                        if let existingPlanItem = existingPlanItemsDict[planItemInput.product.item.name] {
...
//                    let mapper = {HistoryItemMapper.historyItemWith($0)} // TODO loading shared users (when there are shared users) when accessing, crash: BAD_ACCESS, re-test after realm update
    ...
    func addOrUpdateWithIncrement(_ planItemsInput: [PlanItemInput], inventory: DBInventory, _ handler: @escaping ([PlanItem]?) -> Void) {
         fatalError("Outdated") // doesn't compile after introducing Item class. Plan is currently not used so commenting.
//
//        doInWriteTransaction ({[weak self] realm in
//            if let weakSelf = self {
//                
        ...
        doInWriteTransaction ({[weak self] realm in
            if let weakSelf = self {
                ...
                return syncedRet(weakSelf) {
                    ...
                    results = results.filter(NSPredicate(format: "product.name = '\(item.product.item.name)'", argumentArray: []))
